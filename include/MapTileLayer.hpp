...
class MapTileLayer final : public sf::Drawable
{
public:
    using Ptr = std::unique_ptr<MapTileLayer>;

    MapTileLayer(const tmx::Map& map, std::size_t idx)
    {
...
private:
    ...
    public:
        ...
        Chunk(const tmx::TileLayer& layer, std::vector<const tmx::Tileset*> tilesets,
            ...
            std::size_t rowSize, TextureResource& tr, const std::map<std::uint32_t, tmx::Tileset::Tile>& animTiles, unsigned int spacing)
                ...
                for (auto y = yPos; y < yPos + chunkTileCount.y; ++y)
                {
                    for (auto x = xPos; x < xPos + chunkTileCount.x; ++x)
                    {
                            ...
                            && m_chunkTileIDs[idx].ID <= ca->m_lastGID)
                        {
                            ...
                            tileIndex.x *= (ca->tileSetSize.x + tileSpacing);
                            tileIndex.y *= (ca->tileSetSize.y + tileSpacing);
                            Tile tile =
                            {
    ...
    private:
        ...
        public:
                ...
                : m_texture(t)
            {
                ...
                tsTileCount.x = std::ceil(static_cast<float>(texSize.x) / (tileSetSize.x + ts.getSpacing()));
                tsTileCount.y = std::ceil(static_cast<float>(texSize.y) / (tileSetSize.y + ts.getSpacing()));
    ...
    void createChunks(const tmx::Map& map, const tmx::TileLayer& layer)
    {
        ...
        for (auto y = 0u; y < m_chunkCount.y; ++y)
        {
            ...
            for (auto x = 0u; x < m_chunkCount.x; ++x)
            {
                ...
                m_chunks.emplace_back(std::make_unique<Chunk>(layer, usedTileSets,
                    sf::Vector2f(x * m_chunkSize.x, y * m_chunkSize.y), tileCount, tileSize, map.getTileCount().x, m_textureResource, map.getAnimatedTiles(), spacing));
            }
        }
    }
