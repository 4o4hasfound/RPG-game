...
const EnemyConfig MudGuard::s_config{
	100.0f,		// health
	50.0f,		// attack
	0.2f,		// speed
...
MudGuard::MudGuard(PhysicsWorld* world)
	: Enemy(s_config, world) {
	initAnimation();
}

void MudGuard::initAnimation() {
	...
	m_animation.addAnimation(
		Attack1,
		Animation("Characters/Mud Guard/attack 1.png", 23, 80, TopDown),
		[this]() { m_nAtk = 1; m_status.attacking = 0; }
	);
	m_animation.addAnimation(
		Attack2,
		Animation("Characters/Mud Guard/attack 2.png", 23, 80, TopDown),
	...
	m_animation.setUpdateFunction(std::bind(&MudGuard::animationUpdate, this));
	m_animation.addKeyFrameCallback(Attack1, 4, std::bind(&MudGuard::attack, this));
	m_animation.addKeyFrameCallback(Attack1, 5, std::bind(&MudGuard::resetAttack, this));

	m_animation.addKeyFrameCallback(Attack2, 4, std::bind(&MudGuard::attack, this));
	m_animation.addKeyFrameCallback(Attack2, 5, std::bind(&MudGuard::resetAttack, this));
}

void MudGuard::myUpdate(float dt) {
	...
	if (std::abs(m_line.getStart().x - m_line.getEnd().x) < m_attackRangeL[m_nAtk].width() * 0.75f) {
...
void MudGuard::myOnCollide(Body* other, const CollisionDetail& detail) {

}

AnimationType MudGuard::animationUpdate() {
